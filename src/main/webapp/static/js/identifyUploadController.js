...
app.controller('identifyUploadController', ['$scope', 'Upload', '$timeout', function ($scope, Upload, $timeout) {
		...
		for (var i = 0; i < featureLine.length; i++) {
			...
			if (featureName == "POS-Tags") {
				...
				if (feature != featureToDisplay) {
					partOfSpeechCount++;
					addPremadeFeatureToCard(featureToDisplay, featureTestDocCount, featureOtherAuthorsAverageCount, partOfSpeechId, partOfSpeechTitle, "Adjectives, verbs, everything in-between, and their various combinations are a core part of writing style and can be very revealing.", "");
				}
				...
				addPremadeFeatureToCard(featureToDisplay, featureTestDocCount, featureOtherAuthorsAverageCount, "unique-character", 'Unique Characters', "Individuals may use certain characters like parenthesis an abnormal number of times, enough to put them at risk.", "two-columns");
			} else if (featureName == "Misspelled-Words") {
				misspelledWordsCount++;
				if (misspelledWordsCount > 12) {
					continue;
				}
				addToCard(featureStyles, featureTestDocCount, featureOtherAuthorsAverageCount, 'misspelled-words', 'Misspelled Words', "Individuals may misspell certain words frequenly enough to risk being identified by their use.", "two-columns");
				...
				addPremadeFeatureToCard(featureToDisplay, featureTestDocCount, featureOtherAuthorsAverageCount, "punctuation", "Punctuation", "Individuals may use punctuation marks an abnormal number of times, enough to put them at risk (e.g., you end sentences with '?' frequently).", "two-columns");
				...
				addPremadeFeatureToCard(featureStyles, featureTestDocCount, featureOtherAuthorsAverageCount, "word-lengths", "Word Lengths", "Individuals sometimes tend towards longer or shorter words depending on their style, frequently enough to be at risk.", "two-columns");
				...
				addToCard(featureStyles, featureTestDocCount, featureOtherAuthorsAverageCount, 'letters-and-letter-combinations', "Letters", "Certain letters tend to be favored by individuals as part of their writing style (e.g. using 'ing' frequently)", "two-columns");
				...
				addPremadeFeatureToCard(processedWords, featureTestDocCount, featureOtherAuthorsAverageCount, 'words-and-word-combinations', 'Words and Short Phrases', "Certain words and short phrases tend to be reused by individuals as a part of their writing style.", "two-columns");
			}
		}
	...
	function addToCard(featureStyles, featureTestDocCount, featureOtherAuthorsAverageCount, id, title, description, twoColumns) {
	...
	function addPremadeFeatureToCard(feature, featureTestDocCount, featureOtherAuthorsAverageCount, id, title, description, twoColumns) {
		if (document.getElementById(id+"-card") == null) {
			createEmptyCard(id, title, description, twoColumns);
		}
	...
	function addListItem(feature, featureTestDocCount, featureOtherAuthorsAverageCount, id) {
		...
		if (ratio > 0.75) {
			...
			featureString = "<div class=\"feature-thing feature-red-squiggly " + twoColumnClass + "\">~</div> " + featureString;
		}
